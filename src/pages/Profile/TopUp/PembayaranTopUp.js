...
import { getIdTransaksiTopUp, removeIdTransaksiTopUp } from 'utils/cookiesHooks'
import NotFound from 'pages/NotFound'

function PembayaranTopUp() {
  ...
  const getDetailTopUp = async () => {
    await fetcher(`/api/topup/${getIdTransaksiTopUp()}/details`)
      .then((res) => {
  ...
  useEffect(() => {
    ...
    setTopUp(Number(dataTransaksi.amount))
  }, [dataTransaksi])
  ...
  function nominal() {
    if (dataTransaksi.payment_method === 'transfer_manual') {
      let jumlah = IdrFormat(topUp)
      ...
      return <h3 className="text-lg font-medium">{`Rp ${IdrFormat(topUp)}`}</h3>
    }
  }
    ...
    <div className="mx-auto" style={{ maxWidth: '430px' }}>
      <div className="relative">
        <div className="sticky flex items-start top-0 bg-peduly-primary h-[120px] pt-[30px] text-white">
          <Link to={`/profile/${id}`} onClick={() => removeIdTransaksiTopUp()}>
            <svg
              width={24}
              height={24}
              viewBox="0 0 24 24"
        ...
        <div className="rounded-t-[30px] bg-white  w-full relative -top-7 shadow-sm">
          ...
          {dataTransaksi.payment_method !== 'emoney' && (
            <div className="flex justify-between items-center mx-[40px] py-[22px]">
              ...
              <div className="cursor-pointer">
                <CopyToClipboard text={IdrFormat(topUp)} onCopy={handleCopy}>
                  <button className="rounded-full h-[32px] px-[24px] border-[1px] border-peduly-primary text-peduly-primary text-sm font-normal">
                    Salin
                  </button>
                </CopyToClipboard>
                ...
                <div>
                  ...
                  >
                    ...
                    <p className="text-xs font-normal ml-[16px]">
                      ...
                      terakhir supaya mempermudah verifikasi TopUp
                    </p>
                  </div>
                </div>
              )}
          ...
          <div className="mx-[20px]">
            <Link
              to={`/profile/${id}/top-up/pembayaran/instruksi?ir=${getIdTransaksiTopUp()}`}
            >
          ...
          <div
            ...
            onClick={() => {
              history.push({
                pathname: `/profile/${id}/top-up/pembayaran/status/${getIdTransaksiTopUp()}`,
              })
            }}
          >
          ...
          <div className="mx-[20px] space-y-5 mt-[24px]">
            ...
            >
              ...
              <span className="ml-[15px]">
                <h3 className="text-sm font-semibold">TopUp Terlindungi</h3>
