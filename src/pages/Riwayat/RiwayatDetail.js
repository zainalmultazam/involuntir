...
function RiwayatDetail() {
  ...
  const getDetail = async () => {
    await fetcher(`api/donation/histories/${slug}/details`)
      .then((res) => {
        setLoading(true)
        setDetail(res.data.data)
        setTanggal(
          moment(res.data.data.created_at).format('DD-MM-YYYY HH:mm') + ' WIB'
        )
      })
      .catch(() => {
        setError(true)
  ...
  return (
    <div style={{ maxWidth: '430px' }} className="mx-auto">
      <NavLink />
      <BackButton hrf="/riwayat" txt="Riwayat Donasi" />
      {loading ? (
          ...
          {detail.status_donasi === 'Pending' && (
            <div className="text-center ">
              ...
              {detail.status_donasi && detail.status_donasi !== 'Approved' && (
                <Link
                  to={`/riwayat/instruksi?idn=${detail.kode_donasi}`}
              ...
              <div className="col-span-6">
                ...
                >
                  ...
                  <p className="col-span-3 font-normal text-[#717171] truncate">
                    • Rp {IdrFormat(Number(detail.donasi))}
                  </p>
                </div>
          ...
          <div className="mx-[20px] text-[14px] font-normal leading-[19.07px]">
            ...
            <div className="grid grid-cols-12  h-[61px] items-center mx-[7px]">
              ...
              <p className="col-span-7">
                {detail.metode_pembayaran === 'balance'
                  ? 'Dompet Peduly'
