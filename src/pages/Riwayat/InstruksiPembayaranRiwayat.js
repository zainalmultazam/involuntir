...
const InstruksiPembayaranRiwayat = () => {
  ...
  const idTransaksi = queryParams.get('idn')

  const getDetailTransaksi = async () => {
    await fetcher(`${API_URL}/api/donation/transaction/${idTransaksi}`)
      .then(
        (res) =>
          setDataTransaksi(
      ...
      <div className="mt-[72px]">
        <div
          className={`flex justify-between pt-[20px] items-center mx-[40px] ${
            dataTransaksi.metode_pembayaran === 'emoney' ? '' : 'mb-[30px]'
          }`}
        >
          <h3 className="text-sm font-normal">{tipePage}</h3>
          <img src={imagePayment} className="h-[35px]" alt="logo" />
        </div>
            ...
            {(dataTransaksi.metode_pembayaran === 'bank_transfer' ||
              ...
              <div className="flex justify-between items-center mr-[20px]">
                ...
                <div className="cursor-pointer">
                  <CopyToClipboard
                    text={IdrFormat(Number(dataTransaksi.donasi))}
                    onCopy={handleCopy}
                  >
        ...
        >
          <p className="text-sm font-medium">
            {(dataTransaksi.metode_pembayaran === 'bank_transfer' ||
              dataTransaksi.metode_pembayaran === 'transfer_manual') &&
              'BRI Mobile'}
            {dataTransaksi.metode_pembayaran === 'cod' && 'Uang Tunai'}
            {dataTransaksi.metode_pembayaran === 'emoney' &&
...
export default InstruksiPembayaranRiwayat
